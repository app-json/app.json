...
var Heroku = module.exports = (function() {
  ...
  function Heroku(token) {
    ...
    this.client = require("heroku-client").createClient({token: this.token})
  }
  ...
  Heroku.prototype.clone = function(repo_url) {
    var _this = this
    var pollInterval
    var build

    if (!gh(repo_url)) {
      this.emit("error", "repo_url is required and must be a GitHub URL.")
      return
    }
    ...
    var poll = function() {
      ...
      _this.client.get("/app-setups/"+build.id, function(err, b) {
        ...
        case "failed":
          ...
          _this.emit("error", build.failure_message)
          break
        case "succeeded":
          clearInterval(pollInterval)
    ...
    this.client.post("/app-setups/", {source_blob:{url:gh(repo_url).tarball_url}}, function(err, b) {
      if (err) return _this.emit("error", err)
      build = b
      _this.emit("created", build)
